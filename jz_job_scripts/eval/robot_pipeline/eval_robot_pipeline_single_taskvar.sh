...
save_records=false  # Add this parameter here
...
# Function to get recording options based on save_records
get_recording_options() {
    if [ "$save_records" = true ]; then
        echo "--save_obs_outs_dir \"$expr_dir/records/seed\${seed}/\${taskvar}\" \
...
case "$dataset" in
    ...
    "test")
        ...
        do
            recording_opts="$(get_recording_options)"
            case "$mode" in
              "LLM-gt_OG-gt")
                singularity exec --bind "$HOME:$HOME,$WORK:$WORK,$SCRATCH:$SCRATCH" --nv "${sif_image}" \
                    xvfb-run -a "${python_bin}" robot3dlotus/evaluation/eval_robot_pipeline_single_taskvar.py \
