...
def main(args):
    ...
    with jsonlines.open(args.result_file, "r") as f:
        for item in f:
            ckpt_step = (
                int(os.path.basename(item["checkpoint"]).split(".")[0].split("_")[-1])
                if isinstance(item["checkpoint"], str)
                else item["checkpoint"]
            )
    ...
    for taskvar in taskvars:
        ...
        for ckpt in ckpts:
            ...
            if args.aggr_task and args.sr_per_task:
                res.append(
                    np.sum([x[0] for x in ckpt_res]) / np.sum([x[1] for x in ckpt_res])
                )
            else:
    ...
    for k in ckpts:
        ...
        if args.first_avg_task:
            if args.aggr_task:
                ...
                sr = [
                    np.sum([c[0] for c in x]) / np.sum([c[1] for c in x])
                    for x in sr.values()
                ]
            else:
                sr = collections.defaultdict(list)
