...
with open('../../assets/taskvars_target_label_zrange_peract.json', 'r') as f:
    labels = json.load(f)
with open('../../assets/taskvars_instructions_peract.json', 'r') as f:
...
with open(output_file_path, 'w') as f:
    ...
    for task in task_plan.keys():
        ...
        for i, plan in enumerate(task_plan[task]):
            if plan['action'] == 'grasp':
                ...
                if target == "bottom shelf":
                    target = "cupboard"
                f.write(f"{past_obj} = move_grasped_object(target=\"{plan['target']['name']}\")\n")

                # Check if there's a next action and if it's not rotate_grasped_object
                if i == len(task_plan[task]) - 1 or task_plan[task][i + 1]['action'] != 'rotate grasped object':
                    f.write("release()\n")
            elif plan['action'] == 'rotate grasped object':
                f.write(f"{past_obj} = rotate_grasped_object()\n")
            elif plan['action'] == 'push down':
                obj_name = plan['object']['name'].replace(" ", "_")
                f.write(f"{obj_name} = push_down(object=\"{plan['object']['name']}\")\n")
        ...
        if plan['action'] in manipulation_actions:
            if plan['action'] != 'move grasped object' and plan['action'] != 'move grasped object out':
                f.write("release()\n")
        f.write("# done\n\n")
