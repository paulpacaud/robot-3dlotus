...
class Actioner(object):
    def __init__(self, config) -> None:
        ...
        if llm_config.use_groundtruth:
            ...
            self.llm_planner = LlamaTaskPlanner(
                ...
                max_gen_len=None,
                device=None,
                master_port=llm_config.master_port,
        ...
        self.clip_model = ClipEncoder(
            config.clip_path, device=self.device
        )  # to encode action/object texts
        self.motion_planner, self.mp_config = self.build_motion_planner(
        ...
        self.workspace = get_robot_workspace(
            real_robot=config.pipeline.real_robot,
            use_vlm=True,
        )
    ...
    def prepare_motion_planner_input(
        ...
        for k, obj_data in enumerate(vlm_results.objects):
            ...
            # LOGGER.info(f'obj {k}, {len(obj_data.captions)}')
                ...
                if best_obj_id is not None:
                    if query_key == "object":
                        ...
                        if zrange is not None:
                            pcd_label[best_obj_id][
                                pcd_xyz[best_obj_id][:, 2] < zrange[0]
                            ] = 0
                            pcd_label[best_obj_id][
                                pcd_xyz[best_obj_id][:, 2] > zrange[1]
                            ] = 0
                    elif query_key == "target":
                        if target_var_xyz is not None:
                            target_var_xyz = (
                                torch.from_numpy(target_var_xyz)
                                .float()
                                .to(self.device)
                                .unsqueeze(0)
                            )
                            obj_target_var_dists = [
                                ...
                                for obj in vlm_results.objects
        ...
        if rm_robot != "none":
            if rm_robot == "box":
                robot_box = RobotBox(
                    arm_links_info, keep_gripper=False, env_name=self.env_name
                )
            elif rm_robot == "box_keep_gripper":
                robot_box = RobotBox(
                    arm_links_info, keep_gripper=True, env_name=self.env_name
                )
        ...
        if len(pcd_xyz) > num_points:
            ...
            if same_npoints_per_example:
                point_idxs = np.random.choice(
                    pcd_xyz.shape[0], num_points, replace=True
                )
            else:
    ...
    def move_grasped_obj_xyz(self, cur_action, prev_pose, obj_xyz):
        ...
        rotation = R.from_quat(cur_action[3:7]).as_euler("xyz") - R.from_quat(
            prev_pose[3:7]
        ).as_euler("xyz")
    ...
    def predict(
        ...
        if len(cache.valid_actions) > 0:
            ...
            if (
                cache.grasped_obj_name is not None
        ...
        if plan["object"] is not None and "drawer" in plan["object"]:
            obj_height = np.concatenate(
                [
                    obj.pcd_xyz[:, 2]
                    for obj in vlm_results.objects
                    if len(obj.captions) == 0 or obj.captions[0] != "robot"
                ],
                0,
            )
        ...
        if (
            ...
            and "safe" in task_str
            and ("safe" in plan["target"] or "shelf" in plan["target"])
        ):
            obj_height = np.concatenate(
                [
                    obj.pcd_xyz[:, 2]
        ...
        batch, extra_outs = self.prepare_motion_planner_input(
            ...
            instr_include_objects=self.mp_config.TRAIN_DATASET.get(
                "instr_include_objects", False
            ),
            zrange=zrange,
        ...
        pred_actions[:, 2] = np.maximum(
            pred_actions[:, 2], self.workspace["TABLE_HEIGHT"] + 0.005
        )
