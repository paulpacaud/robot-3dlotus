...
class MotionPlannerDatasetPerAct(SimplePolicyDataset):
    def __init__(
    ...
    def __getitem__(self, idx):
        ...
        episode_id = data_id.decode('ascii')

        gt_act_obj_labels = self.gt_act_obj_labels[taskvar][episode_id]
            ...
            for oname in ['object', 'target']:
                if oname in gt_act_obj_labels[keystep]:
                    ...
                    if self.pc_label_type != 'mix':
                        # self.pc_label_type == 'coarse' or 'fine'
                        # if 'fine' is in v but not 'coarse', use 'fine' because some labels have no coarse labels
                        if self.pc_label_type == 'fine':
