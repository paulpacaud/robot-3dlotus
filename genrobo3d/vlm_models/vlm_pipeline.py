...
from .vlm_utils import (
    get_color_palette,
    draw_contour,
    weighted_average_embeds,
)
from genrobo3d.utils.point_cloud import voxelize_pcd
from .vlm_configs import VLMRLBenchConfig, VLMRealConfig
...
class VLMPipeline(object):
    def __init__(
        self,
        sam_path="data/pretrained/facebook/sam-vit-huge",
        owlv2_path="data/pretrained/facebook/owl-v2-large-patch14",
        ...
        self.det_model = Owlv2ObjectDetector(model_path=owlv2_path, device=device)
        print("load detector: %.2fmin" % ((time.time() - start_time) / 60.0))

        start_time = time.time()
        self.sam_model = SAMSegmentor(model_path=sam_path, device=device)
        print("load segmentor: %.2fmin" % ((time.time() - start_time) / 60.0))
    ...
    def run(self, rgb_images, pcd_images, arm_links_info, debug=False):
        """
        Args:
        ...
        input_boxes = [
            (det_results[k]["boxes"].numpy() * box_resize).tolist()
    ...
    def ground_object_with_query(
        self, text, objects=None, debug=False, return_sims=False
    ):
        """
        Args:
            text: a string
        ...
        for obj in objects:
            ...
            it_sims.append((query_embeds * image_class_embeds).sum().item())
    ...
    def classify_objects_with_queries(
        self, texts, objects=None, add_robot_obtacle=True, debug=False
    ):
        """
        Args:
        ...
        for k, obj in enumerate(objects):
            if len(obj.captions) > 0 and obj.captions[0] in [
                "robot",
                "obstacle",
            ]:  # obstacle/robot
            ...
            it_sim = (
                torch.einsum("d,td->t", image_class_embeds, query_embeds)
                .data.cpu()
                .numpy()
            )
            ...
            if debug:
                print(
                    f"obj={k}, all sims {it_sim}, best_label {pred_label_id}={pred_labels[-1]}"
                )
        return pred_labels
    ...
    def clean_det_bboxes(
        ...
        for view_id, (det_res, sam_res, pcd_img) in enumerate(
            zip(det_results, sam_results, pcd_images)
        ):
            ...
            for k in range(len(det_res["boxes"])):
                ...
                inws_mask = np.all(obj_pcd > self.vlm_config.workspace[0], -1) & np.all(
                    obj_pcd < self.vlm_config.workspace[1], -1
                )
                if debug:
                    print("in workspace ratio", np.mean(inws_mask))
                if (
                    1 - np.mean(inws_mask)
                    > self.vlm_config.clean_det_config.max_out_workspace_ratio
                ):
                    continue
            ...
            if len(valid_box_ids) == 0:
                ...
                new_sam_results.append(
                    {k: v[valid_box_ids] for k, v in sam_res.items()}
                )
    ...
    def merge_multiview_objects(
        ...
        all_objects = []
        for view_id, (det_res, sam_res, rgb_img, pcd_img) in enumerate(
            zip(det_results, sam_results, rgb_images, pcd_images)
        ):
            for k, (box, obj_score) in enumerate(
                zip(det_res["boxes"], det_res["scores"])
            ):
                ...
                obj.image_class_embeds = det_image_outputs.image_class_embeds[view_id][
                    k_patch_index
                ].unsqueeze(0)
                ...
                if num_clusters > 1:
                    for label, npoints in label_counter.items():
                        ...
                        if (
                            label != -1
                            and npoints / len(obj.pcd_xyz)
                            > self.vlm_config.dbscan_config.min_keep_ratio
                        ):
                            ...
                            if (
                                len(obj_part.pcd_xyz)
                                > self.vlm_config.pcd_min_num_points
                            ):
        ...
        for view_id, (det_res, sam_res, rgb_img, pcd_img) in enumerate(
            ...
            if np.sum(obstacle_mask) > 0:
                ...
                obstacle.pcd_xyz = np.concatenate(
                    [obstacle.pcd_xyz, pcd_img[obstacle_mask]], 0
                )
                obstacle.pcd_rgb = np.concatenate(
                    [obstacle.pcd_rgb, rgb_img[obstacle_mask]], 0
                )
        ...
        obstacle.pcd_xyz, xyz_idxs = voxelize_pcd(
            obstacle.pcd_xyz, voxel_size=self.vlm_config.voxel_size
        )
        ...
        robot_point_idxs = np.array(
            list(
                robot_box.get_pc_overlap_ratio(
                    xyz=obstacle.pcd_xyz, return_indices=True
                )[1]
            )
        )
        if len(robot_point_idxs) > 0:
            ...
            robot.pcd_xyz, xyz_idxs = voxelize_pcd(
                robot.pcd_xyz, voxel_size=self.vlm_config.voxel_size
            )
                ...
                for label, npoints in label_counter.items():
                    ...
                    if label != -1:
                        ...
                        if np.mean(part_mask) < 0.1:
                            continue
                        src_obj_pcd = (
                            torch.from_numpy(obstacle.pcd_xyz[part_mask])
                            .to(self.device)
                            .unsqueeze(0)
                        )
                        ...
                        for idx, obj in enumerate(all_objects):
                            tgt_obj_pcd = (
                                torch.from_numpy(obj.pcd_xyz)
                                .to(self.device)
                                .unsqueeze(0)
                            )
                            src2tgt_dist = np.sqrt(
                                self.chamfer_dist_fn(
                                    src_obj_pcd,
                                    tgt_obj_pcd,
                                    point_reduction=self.vlm_config.merge_obj_config.chamfer_dist_measure,
                                )[0].item()
                            )
                            tgt2src_dist = np.sqrt(
                                self.chamfer_dist_fn(
                                    tgt_obj_pcd,
                                    src_obj_pcd,
                                    point_reduction=self.vlm_config.merge_obj_config.chamfer_dist_measure,
                        ...
                        if (
                            best_dist
                            < self.vlm_config.merge_obj_config.max_match_pcd_dist
                        ):
                            all_objects[best_idx].pcd_xyz = np.concatenate(
                                [
                                    all_objects[best_idx].pcd_xyz,
                                    obstacle.pcd_xyz[part_mask],
                                ],
                                0,
                            )
                            all_objects[best_idx].pcd_rgb = np.concatenate(
                                [
                                    all_objects[best_idx].pcd_rgb,
                                    obstacle.pcd_rgb[part_mask],
                                ],
                                0,
                            )
            ...
            for obj in all_objects[1:]:
                ...
                for eid, exist_obj in enumerate(merged_objects):
                    ...
                    if obj.view_ids[0] not in exist_obj.view_ids:
                        src_obj_pcd = (
                            torch.from_numpy(obj.pcd_xyz).to(self.device).unsqueeze(0)
                        )
                        tgt_obj_pcd = (
                            torch.from_numpy(exist_obj.pcd_xyz)
                            .to(self.device)
                            .unsqueeze(0)
                        )
                        src2tgt_dist = np.sqrt(
                        ...
                        normed_ft1 = F.normalize(
                            weighted_average_embeds(
                                exist_obj.image_class_embeds,
                                exist_obj.objectness_scores,
                                keepdim=True,
                            ),
                            p=2,
                            dim=-1,
                        )[0]
                        normed_ft2 = F.normalize(
                            obj.image_class_embeds[None, :], p=2, dim=-1
                        )[0]
                        ...
                        if best_matched_obj is None:
                            ...
                            if self.env_name == "rlbench":
                                ...
                                if embed_sim / max(pcd_dist, 0.005) > best_matched_obj[
                                    2
                                ] / max(best_matched_obj[1], 0.005):
                                    best_matched_obj = (eid, pcd_dist, embed_sim)
                            else:
                                # weight embed_sim more
                                if embed_sim / max(pcd_dist, 0.01) > best_matched_obj[
                                    2
                                ] / max(best_matched_obj[1], 0.01):
                                    best_matched_obj = (eid, pcd_dist, embed_sim)
                ...
                if best_matched_obj is not None and (
                    (
                        best_matched_obj[2]
                        > self.vlm_config.merge_obj_config.min_match_embed_sim
                        and best_matched_obj[1]
                        < self.vlm_config.merge_obj_config.max_match_pcd_dist
                    )
                    ...
                    exist_obj.pcd_xyz = np.concatenate(
                        [exist_obj.pcd_xyz, obj.pcd_xyz], 0
                    )
                    exist_obj.pcd_rgb = np.concatenate(
                        [exist_obj.pcd_rgb, obj.pcd_rgb], 0
                    )

                    if debug:
                        print(
                            f"\tmerged to obj {best_matched_obj[0]}, (view_id={exist_obj.view_ids[0]}, obj_id={exist_obj.obj_ids[0]})"
                        )

                    exist_obj.pcd_xyz, xyz_idxs = voxelize_pcd(
                        exist_obj.pcd_xyz, self.vlm_config.voxel_size
                    )
                    exist_obj.pcd_rgb = exist_obj.pcd_rgb[xyz_idxs]

                    exist_obj.image_class_embeds = torch.cat(
                        [exist_obj.image_class_embeds, obj.image_class_embeds], 0
                    )
                    exist_obj.objectness_scores = torch.cat(
                        [exist_obj.objectness_scores, obj.objectness_scores], 0
                    )
        ...
        if (
            robot.pcd_xyz is not None
            and len(robot.pcd_xyz) > self.vlm_config.pcd_min_num_points
        ):
        ...
        for i, (sam_res, rgb_img) in enumerate(zip(sam_results, rgb_images)):
            ...
            if sam_res is None:
                ...
                for k, (s, m) in enumerate(zip(sam_res["scores"], sam_res["masks"])):
                    ...
                    if use_contour:
                        rgb_img = draw_contour(
                            rgb_img,
                            m,
                            COLOR_PALETTE[np.random.randint(len(COLOR_PALETTE))],
                        )
                        # plt.imshow(m[0])
                        # plt.show()
    ...
    def clean_object_pcd(self, pcd_xyz, robot_box=None):
        ...
        pcd_masks = np.all(pcd_xyz > self.vlm_config.workspace[0], -1) & np.all(
            pcd_xyz < self.vlm_config.workspace[1], -1
        )

        # remove points inside robot_box
        if robot_box is not None:
            _, overlap_indices = robot_box.get_pc_overlap_ratio(
