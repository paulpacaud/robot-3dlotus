...
class LlamaTaskPlanner(object):
    def __init__(
        ...
        self.taskvars_train_file = taskvars_train_file
        self.taskvars_instructions_file = taskvars_instructions_file
    ...
    def load_prompts(self):
        self.system_prompt = (
            open(os.path.join(self.prompt_dir, "system_prompt.txt"))
            .readlines()[0]
            .strip()
        )
        self.user1 = "".join(
            open(os.path.join(self.prompt_dir, "planner_prompts.txt")).readlines()
        )
    ...
    def load_in_context_examples(self):
        ...
        data = [
            x.strip() for x in open(example_file, "r").readlines() if len(x.strip()) > 0
        ]
        taskvar_examples = {}
        for line in data:
    ...
    def load_instruction_embeds(self):
        trn_taskvars = set(json.load(open(self.taskvars_train_file)))
        self.taskvar_instructions = {
            taskvar: [instr + "." for instr in instrs]
            for taskvar, instrs in json.load(
                open(self.taskvars_instructions_file)
            ).items()
    ...
    def __call__(self, query, context=None, topk=20, verbose=False):
        ...
        sims = einops.einsum(
            query_sent_embeds, self.trn_instr_embeds, "q d, p d -> q p"
        )[0]
        ...
        if self.use_local_llama:
            ...
            results = (
                self.generator.chat.completions.create(
                    messages=dialogs[0],
    ...
    def estimate_height_range(self, target_name, obj_height):
        ...
        results = [
            line for line in results if (not line.startswith("#")) and len(line) > 0
        ]
        try:
