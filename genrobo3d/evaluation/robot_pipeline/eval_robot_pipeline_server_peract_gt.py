...
from genrobo3d.evaluation.robot_pipeline.robot_pipeline_gt_peract import (
    GroundtruthRobotPipeline,
)
from genrobo3d.evaluation.robot_pipeline.robot_pipeline import RobotPipeline
...
def producer_fn(
    ...
    result_queue,
    producer_queue,
    shutdown_event,  # Added shutdown_event
):
    try:
        ...
        if args.microstep_data_dir != "":
            episodes_dir = os.path.join(
                args.microstep_data_dir, task_str, f"variation{variation}", "episodes"
            )
        ...
        for demo_id in range(num_demos):
            ...
            for step_id in range(args.max_steps):
                ...
                if shutdown_event.is_set():
                    LOGGER.info(
                        f"Shutdown event set during steps. Exiting producer {proc_id}."
                    )
                    break

                # Fetch the current observation and predict one action
                LOGGER.info(
                    f"Step= {step_id}, taskvar={taskvar}, demo_id= {demo_id}, episode_id= {evaluated_episode_ids[demo_id]}"
                )
            ...
            if shutdown_event.is_set():
                LOGGER.info(
                    f"Shutdown event set after steps. Exiting producer {proc_id}."
                )
                break
        ...
        avg_inference_time = (
            total_inference_time / total_inference_steps
        ...
        LOGGER.info(
            ...
            f"Estimated SR: {success_rate * num_demos / (demo_id+1):.2f} "
        ...
        LOGGER.info(
            colored(f"Taskvar: {taskvar} SR: {success_rate:.2f}", "black", "on_yellow")
        )
        ...
        try:
            ...
            LOGGER.error(
                f"Producer {proc_id}: Error during cleanup: {e}", exc_info=True
            )
...
def main():
    try:
        ...
        consumer = mp.Process(
            target=consumer_fn, args=(args, pipeline_config, batch_queue, result_queues)
        )
        consumer.start()
        ...
        while i < len(taskvars):
            taskvar = taskvars[i]
            LOGGER.info(
                f"len(producers) < args.num_workers: {len(producers)} < {args.num_workers}"
            )
        ...
        LOGGER.info(
            f"All tasks launched. Waiting for final {len(producers)} producers to finish..."
        )
        producer_statuses = {
            pid: {"name": producer.name, "status": "running", "start_time": time.time()}
        ...
        producer_list = [
            format_producer_info(pid, info) for pid, info in producer_statuses.items()
        ]
        ...
        while len(producers) > 0:
            try:
                ...
                proc_id, k_res = producer_queue.get(
                    timeout=600
                )  # 10 minutes timeout for one taskvar
                LOGGER.info(
                    f"Producer {proc_id} ({producer_statuses[proc_id]['name']}) signaled completion"
                )
                ...
                if proc_id in producers:
                    ...
                    if producers[proc_id].is_alive():
                        ...
                        if producers[proc_id].is_alive():
                            LOGGER.warning(
                                f"Producer {proc_id} ({producer_statuses[proc_id]['name']}) did not exit after shutdown event, terminating..."
                            )
                            producers[proc_id].terminate()
                            producers[proc_id].join()
                            producer_statuses[proc_id]["status"] = "forced_terminated"
                        else:
                            producer_statuses[proc_id][
                                "status"
                    ...
                    elapsed_time = (
                        time.time() - producer_statuses[proc_id]["start_time"]
                    )
                    LOGGER.info(
                        f"Producer {proc_id} ({producer_statuses[proc_id]['name']}) finished with status "
                    ...
                    LOGGER.warning(
                        f"Received completion signal from unknown producer {proc_id}"
                    )

            except (queue.Empty, TimeoutError):  # Handle both possible exceptions
                LOGGER.warning(
                    "Timeout waiting for producers. Checking process states..."
                )
                # Check for any dead processes that didn't signal completion
                for pid in list(producers.keys()):
                    if not producers[pid].is_alive():
                        LOGGER.warning(
                            f"Found dead producer {pid} ({producer_statuses[pid]['name']}), cleaning up..."
                        )
                        producers[pid].join(timeout=10)
                        producer_statuses[pid]["status"] = "timeout_dead"
                        elapsed_time = (
                            time.time() - producer_statuses[pid]["start_time"]
                        )
                        LOGGER.info(
                            f"Producer {pid} ({producer_statuses[pid]['name']}) finished with status "
                        ...
                        if producers[pid].is_alive():
                            LOGGER.warning(
                                f"Producer {pid} ({producer_statuses[pid]['name']}) did not exit after shutdown event, terminating..."
                            )
        ...
        for pid, info in producer_statuses.items():
            ...
            LOGGER.info(
                f"Producer {pid} ({info['name']}): status='{info['status']}', runtime={elapsed_time:.1f}s"
            )

        completion_counts = {
            status: len(
                [p for p in producer_statuses.values() if p["status"] == status]
            )
            for status in [
                "completed",
                "gracefully_terminated",
        ...
        for pid, producer in producers.items():
            if producer.is_alive():
                ...
                if producer.is_alive():
                    LOGGER.warning(
                        f"Producer {pid} did not exit after shutdown event, force terminating..."
                    )
                    producer.terminate()
                    producer.join(timeout=10)
