...
class GroundtruthVision(object):
    ...
    def __call__(
        ...
        LOGGER.info(f'vlm pipeline activating mask. Initial number of points: {len(pcd_xyz)}')
        pcd_xyz = pcd_xyz[fg_mask]
        pcd_sem = pcd_sem[fg_mask]
        LOGGER.info(f'vlm pipeline activating mask. Number of points after mask: {len(pcd_xyz)}')
        if self.use_color:
...
class GroundtruthRobotPipeline(object):
    ...
    def predict(self, task_str, variation, step_id, obs_state_dict, episode_id, instructions, cache=None):
        LOGGER.info(f'### step_id={step_id}, task={task_str}, variation={variation}')
        ...
        # LOGGER.info(f'highlevel step id {cache.highlevel_step_id}')
        ...
        LOGGER.info(f'highlevel plan step {cache.highlevel_step_id}, action {plan["action"]}')
        LOGGER.info(f"Preprocessing obs for batch coarse")

        batch_coarse = self.coarse_preprocess_obs(taskvar, cache.highlevel_step_id_norelease, obs_state_dict, plan)

        LOGGER.info(f"Predicting action for batch coarse")
        pred_actions = self.predict_action(batch=batch_coarse, model=self.motion_planner_coarse)

        LOGGER.info(f'pred actions {pred_actions}')
        point_of_interest = pred_actions[0][:3]

        LOGGER.info(f"using point of interest {point_of_interest}, preparing batch fine")
        ...
        LOGGER.info(f"Predicting action for batch fine")
