...
from genrobo3d.evaluation.robot_pipeline.robot_pipeline_gt import (
    GroundtruthRobotPipeline,
)
from genrobo3d.evaluation.robot_pipeline.robot_pipeline import (
    RobotPipeline,
)
...
class ServerArguments(tap.Tap):
    ...
    taskvar_file: str = "assets/gembench/taskvars_train.json"
    num_demos: int = 20
...
def consumer_fn(args, pipeline_config, batch_queue, result_queues):
    ...
    while True:
        ...
        LOGGER.info(f"get data from batch_queue")
        if data is None:
        ...
        LOGGER.info(f"put data to result_queue: {out['action']}")
        result_queues[k_prod].put(out)
        LOGGER.info("put data to result_queue done")
...
def producer_fn(
        ...
        if os.path.exists(str(episodes_dir)):
            ...
            for idx, ep in enumerate(episode_ids):
                try:
                    ...
                    LOGGER.info(f"\tProblem to load demo_id: {idx} from {e}")
                    LOGGER.info(e)
        if len(demos) == 0:
            LOGGER.info(f"{taskvar} does not need to be evaluated.")
            return
    ...
    for demo_id in range(num_demos):
        ...
        for step_id in range(args.max_steps):
            LOGGER.info(f"Step= {step_id} , taskvar={taskvar}, demo_id= {demo_id}")
            ...
            try:
                ...
                LOGGER.info(
                    f"Error: {e} for taskvar {taskvar} demo {demo_id} step {step_id}"
                )
                reward = 0
                break
    ...
    avg_inference_time = (
        total_inference_time / total_inference_steps if total_inference_steps > 0 else 0
    )
