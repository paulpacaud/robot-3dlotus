...
from genrobo3d.evaluation.simple_policy.eval_simple_policy import Actioner
        ...
        if os.path.exists(str(episodes_dir)):
            ...
            for idx, ep in enumerate(episode_ids):
                try:
                    ...
                    LOGGER.info(f"Problem to load demo_id: {idx} {ep}")
                    LOGGER.info(e)
        if len(demos) == 0:
            LOGGER.info(f"{taskvar} does not need to be evaluated.")
            return
    ...
    for demo_id in range(num_demos):
        ...
        for step_id in range(args.max_steps):
            ...
            try:
                ...
                LOGGER.info(f"Error in {taskvar} demo {demo_id} step {step_id}: {e}")
                reward = 0
                break
        ...
        LOGGER.info(
            f"{taskvar} Demo {demo_id} Step {step_id+1} Reward {reward} "
...
def main():
    ...
    if not os.path.exists(args.checkpoint):
        LOGGER.info(f"{args.checkpoint} not exists")
        return
    ...
    while i < len(taskvars):
        ...
        if len(producers) < args.num_workers:
            LOGGER.info(f"Starting producer {i} for taskvar {taskvar}")
            producer = mp.Process(
                target=producer_fn,
                args=(
                    i,
