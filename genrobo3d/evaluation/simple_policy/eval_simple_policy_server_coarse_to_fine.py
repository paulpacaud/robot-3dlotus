...
from genrobo3d.evaluation.simple_policy.eval_simple_policy_coarse_to_fine import (
    Actioner,
)
...
def producer_fn(
    proc_id, k_res, args, taskvar, pred_file, batch_queue, result_queue, producer_queue
):
        ...
        if os.path.exists(str(episodes_dir)):
            ...
            for idx, ep in enumerate(episode_ids):
                try:
                    ...
                    LOGGER.info(f"\tProblem to load demo_id: {idx} from {episodes_dir}")
                    LOGGER.info(e)
        if len(demos) == 0:
            LOGGER.info(f"{taskvar} does not need to be evaluated.")
            return
    ...
    for demo_id in range(num_demos):
        ...
        for step_id in range(args.max_steps):
            ...
            try:
                ...
                LOGGER.info(f"Error in step {step_id}: {e}")
                reward = 0
                break
        ...
        LOGGER.info(
            ...
            f"num_demos: {num_demos} ckpt: {args.checkpoint}"
        )
...
def main():
    ...
    args.exp_config_coarse = os.path.join(
        args.coarse_model_dir, "logs", "training_config.yaml"
    )
    args.checkpoint_coarse = os.path.join(
        args.coarse_model_dir, "ckpts", f"model_step_{args.ckpt_step_coarse}.pt"
    )
    if not os.path.exists(args.checkpoint):
        LOGGER.info(f"Checkpoint {args.checkpoint} does not exist.")
        return

    if not os.path.exists(args.checkpoint_coarse):
        LOGGER.info(f"Checkpoint {args.checkpoint_coarse} does not exist.")
        return
    ...
    if os.path.exists(pred_file):
        with jsonlines.open(pred_file, "r") as f:
            for item in f:
                item_step = int(
                    os.path.basename(item["checkpoint"]).split(".")[0].split("_")[-1]
                )
    ...
    LOGGER.info(f"checkpoint {args.ckpt_step} #taskvars {len(taskvars)}")
    ...
    while i < len(taskvars):
        ...
        if len(producers) < args.num_workers:
            LOGGER.info(f"Starting {taskvar} {i}/{len(taskvars)}")
            producer = mp.Process(
                target=producer_fn,
                args=(
                    i,
