...
class Actioner(object):
    def __init__(self, args) -> None:
        ...
        self.config = config
        self.config.defrost()
        self.config.TRAIN_DATASET.sample_points_by_distance = (
            self.config.TRAIN_DATASET.get("sample_points_by_distance", False)
        )
        self.config.TRAIN_DATASET.rm_pc_outliers = self.config.TRAIN_DATASET.get(
            "rm_pc_outliers", False
        )
        self.config.TRAIN_DATASET.rm_pc_outliers_neighbors = (
            self.config.TRAIN_DATASET.get("rm_pc_outliers_neighbors", 10)
        )
        self.config.TRAIN_DATASET.same_npoints_per_example = (
            self.config.TRAIN_DATASET.get("same_npoints_per_example", False)
        )
        ...
        # config.MODEL.ptv3_config.enable_flash = False
    ...
    def process_point_clouds(
        ...
        if self.data_cfg.rm_robot.startswith("box"):
            mask = self._get_mask_with_robot_box(
    ...
    def preprocess_obs(self, taskvar, step_id, obs):
        ...
        pc_ft, pc_centroid, pc_radius, ee_pose = self.process_point_clouds(
            ...
            rgb,
            gt_sem=gt_sem,
            ee_pose=copy.deepcopy(obs["gripper"]),
        ...
        if self.args.save_obs_outs_dir is not None:
            np.save(
                os.path.join(
                    ...
                    f"{task_str}+{variation}-{episode_id}-{step_id}.npy",
                ),
                {
                    "batch": {
...
def evaluate_actioner(args):
    ...
    if os.path.exists(outfile):
        with jsonlines.open(outfile, "r") as f:
            for item in f:
                existed_data.add(
                    (item["checkpoint"], "%s+%d" % (item["task"], item["variation"]))
                )
