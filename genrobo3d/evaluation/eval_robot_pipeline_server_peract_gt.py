    ...
    if args.microstep_data_dir != '':
        ...
        if os.path.exists(str(episodes_dir)):
            ...
            for idx, ep in enumerate(episode_ids):
                LOGGER.info(f'checking {task_str}+{variation}, episode: {ep} in gt_labels')

                if taskvar not in gt_labels:
                    LOGGER.info(f'{taskvar} not in gt_labels; skipping')
                    continue

                if ep not in gt_labels[taskvar]:
                    LOGGER.info(f'{taskvar} episode {ep} not in gt_og_label_file; skipping')
                    continue
